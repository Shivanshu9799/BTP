...
def main():
    ...
    raw_files_a = sum(len(v) for v in barcodes_a.values())
    multi_a = {k: v for k, v in barcodes_a.items() if len(v) > 1}
    print(f"  -> {raw_files_a} raw files -> {len(barcodes_a)} unique barcodes ({args.level}-level)")
    print(f"  -> {len(multi_a)} barcodes have multiple files (multi-slide/portion samples)")
    if skipped_a:
        print(f"  -> WARNING: {len(skipped_a)} files didn't match TCGA barcode pattern, skipped: {skipped_a[:5]}")

    print(f"Scanning {args.label_b}: {args.folder_b}")
    barcodes_b, skipped_b = collect_barcodes(args.folder_b, level=args.level)
    raw_files_b = sum(len(v) for v in barcodes_b.values())
    multi_b = {k: v for k, v in barcodes_b.items() if len(v) > 1}
    print(f"  -> {raw_files_b} raw files -> {len(barcodes_b)} unique barcodes ({args.level}-level)")
    print(f"  -> {len(multi_b)} barcodes have multiple files (multi-slide/portion samples)")
    if skipped_b:
        print(f"  -> WARNING: {len(skipped_b)} files didn't match TCGA barcode pattern, skipped: {skipped_b[:5]}")
