...
def main():
    ...
    # --- write paired manifest ---
    with open(args.out, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(["barcode", "wsi_primary", "st_primary", "wsi_count", "st_count", "wsi_files", "st_files"])
        for bc in paired:
            wsi_files = sorted(wsi[bc])
            st_files = sorted(st[bc])
            wsi_primary = pick_primary(wsi_files, prefer=args.prefer)
            st_primary = st_files[0]  # normally only one ST file per sample
            writer.writerow([
                bc,
                os.path.basename(wsi_primary),
                os.path.basename(st_primary),
                len(wsi_files),
                len(st_files),
                ';'.join(os.path.basename(x) for x in wsi_files),
                ';'.join(os.path.basename(x) for x in st_files),
            ])
    print(f"\nPaired manifest written: {args.out}  ({len(paired)} rows)")
